...
def remove_js_block_comments(lines):
    cleaned = []
    inside_block = False
...
def strip_inline_js_comment(line):
    in_single = False
    in_double = False
    in_template = False
    escaped = False
...
def scan_javascript_files(repo_path, rules):
    findings = []
    ...
    for root, _, files in os.walk(repo_path):
        for file in files:
            ...
            with open(full_path, "r", errors="ignore") as f:
                lines = f.readlines()
                lines = remove_js_block_comments(lines)
            ...
            for i, line in enumerate(lines):
                ...
                line = strip_inline_js_comment(line)
                ...
                for rule in rules:
                    ...
                    if "contains" in rule:
                        ...
                        if not any(
                            c.lower() in line_lower for c in rule["contains"]
                        ):
                            continue
